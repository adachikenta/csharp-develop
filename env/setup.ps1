...
$dotnetPath = "$env:USERPROFILE\scoop\apps\dotnet-sdk\current\dotnet.exe"
$dotnetInstalled = Install-ScoopPackage -PackageName "dotnet-sdk" -ExecutablePath $dotnetPath
if (-not $dotnetInstalled) {
    Write-Host "Failed to install dotnet-sdk. Exiting..." -ForegroundColor Red
    exit 1
}
# Reload environment variables to make scoop available in current session
Write-Host "Reloading environment variables..." -ForegroundColor Yellow
